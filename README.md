...
```bash
duck01$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
duck01$ sam local start-api
duck01$ curl http://localhost:3000/
```
...
```bash
duck01$ sam logs -n HelloWorldFunction --stack-name "duck01" --tail
```
...
```bash
duck01$ pip install pytest pytest-mock --user
duck01$ python -m pytest tests/ -v
```
